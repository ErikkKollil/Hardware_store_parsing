#https://curlconverter.com/ #Convert curl commands to Python
import requests
import json
import pprint
...
def get_data():
    serach = 'https://www.mvideo.ru/product-list-page/f/skidka=da?q=ps5+%D0%B8%D0%B3%D1%80%D0%B0&category=ps5-igry-22780'
    cookies = {
        ...
        'MVID_CITY_ID': 'CityCZ_7173',  # Город: Воронеж, Москва - CityCZ_975
    ...
    response = requests.get('https://www.mvideo.ru/bff/products/statuses', params=params, cookies=cookies,
                            headers=headers).json()

    with open('5_available.json', 'w') as file:
        json.dump(response, file, indent=4, ensure_ascii=False)
...
def get_result():
    ...
    with open('5_available.json') as file:
        product_available = json.load(file)

    products_data = product_data.get('body').get('products')
    products_status = product_available.get('body').get('statuses')
    ...
    for item in products_data:
        ...
        for el in products_status:
            if product_id == el.get('productId'):
                item['item_available'] = el.get('productStatus')

    with open('10_result.json', 'w') as file:
        json.dump(products_data, file, indent=4, ensure_ascii=False)


def base_info():
    info_list = []
    with open('10_result.json') as file:
        product_data = json.load(file)

    for item in product_data:
        if item.get('item_available') == 'Available':
